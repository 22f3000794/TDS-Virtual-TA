...
model = NomicEmbeddings(model="nomic-embed-text-v1", nomic_api_key=NOMIC_API_KEY)
...
async def answer_question(req: QueryRequest):
    ...
    If the context includes a response from a TA or instructor, summarize it accurately but not too much. For answers from discourse, tell whatever the TAs or the instructors have answered and give correct url. 
